    ...
    for tag in read.tags:
        ...
        if tagname == 'RG':
            if tagval not in IDRG:
                sys.stderr.write("read group: " + tagval + " not found!\n")
...
def makebam(sam, fai, threads, mem):
    ...
    cmd = ['samtools', 'sort', '-m', str(mem), '-@', str(threads), outbam, outsort]
    outsort += '.bam'
...
def main(args):
    ...
    if len(paired.keys()) > 0:
        sys.stderr.write("WARNING: found " + str(len(paired.keys())) + " orphaned paired reads that were not output!\n") 
